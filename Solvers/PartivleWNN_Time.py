# /*
#  * @Author: yaohua.zang 
#  * @Date: 2023-09-29 12:12:39 
#  * @Last Modified by:   yaohua.zang 
#  * @Last Modified time: 2023-09-29 12:12:39 
#  */
import numpy as np 
import torch
import time
...
import os
#
from Network.Network_Time import Model
...
class Solver(Module.Solver):
    ...
    def __init__(self, problem:Problem,
                 Nt_particle:int, Nx_particle:int, 
                 Nt_bd:int, Nx_bd_each_face:int,
                 Nx_init:int, Nx_int:int, 
                 R_max:int, R_min:int,
        ...
        Input:
            problem: 
            Nt_particle: the number of time slices for particles
            Nx_particle: the number of particles in each time slice
            Nt_bd: the number of time slices for boundary points
            Nx_bd_each_face: the number of points on the boundary (each time slice)
            Nx_init: the number of points for initial conditions
            Nx_int: the number of meshgrids (or meshsize) for computing integration
        ...
        #
        self.Nt_particle = Nt_particle
        self.Nx_particle = Nx_particle
        self.Nt_bd = Nt_bd
        self.Nx_bd_each_face = Nx_bd_each_face
        ...
        self.tloc_list = kwargs['tloc_list']
    ...
    def predict(self, load_path:str, load_type:str)->None:
        ...
        # prediction
        u_test, x_test, t_test = self.problem.get_test()
        with torch.no_grad():
            u_pred = self.model(x_test.to(device), t_test.to(device))
        # save result
        dict_test = {}
    ...
    def get_loss(self, data_point:dict, **args):
        '''
        Input:
            data_point: dict= {'x_init', 't_init', 
                            'x_bd_list', 't_bd', 
                            'xc_list', 'tc_list', 'R_list'}
            args: 'x_mesh', 'phi', 'dphi_scaled'
        Output:
            loss_train:
            loss_all: [loss_in, loss_bd, loss_init]
            score:
        '''
        loss_all = []
        ############
        x_bd_list = [item.to(device) for item in data_point['x_bd_list']]
        t_bd = data_point['t_bd'].to(device)
        cond_bd = self.problem.fun_bd(model=self.model, x_list=x_bd_list, t=t_bd)
        #
        loss_bd = torch.mean( cond_bd**2 ) 
        ############ 
        x_init = data_point['x_init'].to(device)
        t_init = data_point['t_init'].to(device)
        cond_init = self.problem.fun_init(model=self.model, x=x_init, t=t_init)
        #
        loss_init = torch.mean( cond_init**2 ) 
        ########### 
        eq_list = []
        for xc, tc, R in zip(data_point['xc_list'], data_point['tc_list'], data_point['R_list']):
            eq = self.problem.weak_particlewnn(model=self.model, xc=xc.to(device), 
                                               tc=tc.to(device), R=R.to(device),
        ...
        #
        try:
            eq_topk, index = torch.topk(torch.cat(eq_list, dim=0), k=self.topK, dim=0)
        except:
            eq_topk = torch.cat(eq_list, dim=0)
        #
        loss_eq = torch.mean( eq_topk )
        ##############
        loss_all.append([loss_eq.detach(), loss_bd.detach(), loss_init.detach()])
        #
        loss_train = (loss_eq * self.loss_weight['eq'] 
                      + loss_bd * self.loss_weight['bd'] 
                      + loss_init * self.loss_weight['init'])
    ...
    def train(self,save_path:str, load_path:str=None, 
        ...
        x_mesh, phi, dphi_scaled = self.problem.get_testFun_particlewnn(
            ...
            Nx_mesh=self.Nx_int, int_method=self.int_method)
        ############################################## 
        iter = 0
        ...
        for iter in range(self.maxIter):
            ####### 
            R_adaptive = self.R_min  + (self.R_max-self.R_min) * iter/self.maxIter # Rmax递减
            data_train = self.problem.get_point_particlewnn(
                GenData=self.problem._gen_data, Nt_in=self.Nt_particle, 
                Nx_in=self.Nx_particle, Nt_bd=self.Nt_bd, 
                Nx_bd_each_face=self.Nx_bd_each_face, Nx_init=self.Nx_init,
                **{'Rmax': R_adaptive, 'Rmin':self.R_min, 
                   'tloc_list':self.tloc_list})
            #
            ...
            #
            err, _ = Error().L2_error_Time(
                model=self.model, x=x_test.to(device),
                t=t_test.to(device), u=u_test.to(device))
