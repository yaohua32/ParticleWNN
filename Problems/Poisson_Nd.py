# /*
#  * @Author: yaohua.zang 
#  * @Date: 2023-09-27 11:59:41 
#  * @Last Modified by:   yaohua.zang 
#  * @Last Modified time: 2023-09-27 11:59:41 
#  */
import numpy as np 
import torch 
...
class Problem(Module.Problem):
    ...
    def __init__(self, dtype:np.dtype=np.float64, 
        ...
        '''
        The Nd Poisson's problem:
                -div( grad(u) )  = f(x,y)    in [-1,1]^d
        Input:
            dtype: np.float
            testFun_type: str='Wendland'
            args: {'dim':default=5, 'freq':default=np.pi}
        '''
        try:
            self._dim = args['dim']
        ...
        try:
            ...
            self._freq = np.pi
        #
        self._name = 'poisson_Nd'
        self._dtype = dtype
        #
        self._lb = - np.ones((self._dim,))
    ...
    def get_test(self, x:torch.tensor=None)->torch.tensor:
        ...
        if x is not None:
            ...
            for d in range(self.dim-1):
                u += x[:,d:d+1] * torch.sin(self._freq * x[:,d+1:d+2])
            return u
        elif self.dim==5:
            x_list = [np.linspace(self._lb[inx], self._ub[inx], 5) for inx in range(self.dim)]
            mesh_list = np.meshgrid(x_list[0], x_list[1], x_list[2], x_list[3], x_list[4])
            mesh_list = [mesh.reshape(-1,1) for mesh in mesh_list]
            x = np.concatenate(mesh_list, axis=1)
            ...
            raise NotImplementedError(f'No implementation for d={self.dim}')
    ...
    def fun_f(self, x:torch.tensor)->torch.tensor:
        ...
        f = 0.
        for d in range(self.dim-1):
            f += self._freq**2 * (x[:,d:d+1] * torch.sin(self._freq * x[:,d+1:d+2]) )
        #
        return f
